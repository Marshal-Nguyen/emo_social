...
import { postService } from "../../services/postService";
import { tagService } from "../../services/apiService";
import tagEmotions from "../../data/tagEmotions1.json"; // Import file JSON
...
const getCategoryColor = (categoryName) => {
    switch (categoryName) {
        ...
        case "Cảm xúc tiêu cực mạnh":
            return {
                bgLight: "bg-red-200",
                bgDark: "bg-red-800/30",
            ...
            <div className="bg-gradient-to-br from-white to-purple-50 dark:from-gray-800 dark:to-purple-900/20 rounded-xl p-4 border border-purple-200 dark:border-purple-700 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                        ...
                        <div className="text-left">
                            <div
                                className={`text-sm font-medium ${emotionId ? "text-purple-800 dark:text-purple-200" : "text-gray-800 dark:text-gray-200"
                                    }`}
                            >
                                {selectedEmotion ? (selectedEmotion.displayNameVi || selectedEmotion.displayName) : "Cảm xúc"}
                            </div>
                            <div
                                className={`text-xs ${emotionId ? "text-purple-600 dark:text-purple-300" : "text-gray-500 dark:text-gray-400"
                                    }`}
                            >
                                {emotionId ? "Đã chọn" : "Chọn cảm xúc"}
                            </div>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md max-h-[80vh] overflow-hidden shadow-2xl">
                    <div
                        className={`flex items-center justify-between p-4 border-b ${getCategoryColor(emotionCategories[currentEmotionCategoryIndex].categoryName).borderLight
                            ...
                            } bg-gradient-to-r from-white to-${getCategoryColor(emotionCategories[currentEmotionCategoryIndex].categoryName).bgLight
                            } dark:from-purple-900/20 dark:to-${getCategoryColor(emotionCategories[currentEmotionCategoryIndex].categoryName).bgDark
                            }`}
                    >
                        <div className="flex items-center space-x-3">
                            <div className="p-2 bg-purple-500 rounded-lg">
                            ...
                            <h2
                                className={`text-lg font-semibold ${getCategoryColor(emotionCategories[currentEmotionCategoryIndex].categoryName).textLight
                                    } dark:${getCategoryColor(emotionCategories[currentEmotionCategoryIndex].categoryName).textDark
                                    }`}
                            >
                    ...
                    <div className="p-4">
                        ...
                        <AnimatePresence mode="wait">
                            <motion.div
                                ...
                                className={`grid grid-cols-4 gap-2 p-4 rounded-xl ${getCategoryColor(emotionCategories[currentEmotionCategoryIndex].categoryName).bgLight
                                    } dark:${getCategoryColor(emotionCategories[currentEmotionCategoryIndex].categoryName).bgDark}`}
                            >
                                {emotionCategories[currentEmotionCategoryIndex].emotions.map((emo) => {
                                    ...
                                    return (
                                        <motion.button
                                            ...
                                            className={`flex flex-col items-center space-y-1 px-2 py-3 rounded-xl text-xs font-medium transition-all duration-200 ${isSelected
                                                ? "bg-purple-500 text-white shadow-lg"
                                                : `bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 border ${getCategoryColor(emotionCategories[currentEmotionCategoryIndex].categoryName)
                                                    .borderLight
                                        ...
                                        >
                                            <motion.span
                                                ...
                                                className="text-lg"
                                            >
                                                {getUnicodeEmoji(emo.unicodeCodepoint)}
                                            </motion.span>
...
const CreatePost = () => {
    ...
    const [showEmotionModal, setShowEmotionModal] = useState(false);
    const [categoryTags, setCategoryTags] = useState([]);
    const [emotionCategories, setEmotionCategories] = useState(tagEmotions.emotionCategories); // Sử dụng JSON
    const [loadingTags, setLoadingTags] = useState(false);
    const [categorySearch, setCategorySearch] = useState("");
    const [pendingSubmit, setPendingSubmit] = useState(false);
